...
export const PlayerDesktop = React.memo(
  ({
    ...
    const setPlayerShuffle = useCallback(
      (shuffle: boolean) => dispatch(setShuffle(shuffle)),
      [dispatch],
    );
    const setPlayerRepeat = useCallback(
      (repeat: boolean) => dispatch(setRepeat(repeat)),
      [dispatch],
    );
    const stopPlayerFunc = useCallback(
      () => dispatch(playerStop()),
      [dispatch],
    );
          ...
          {!isOpen ? (
            ...
            >
                      ...
                      <AnimatePresence mode="wait">
                        ...
                        >
                          {(playerState.loading || !playerState.playing) && (
                            <div className="absolute inset-0 flex items-center bg-black/20 text-white justify-center z-[10] rounded-xl">
                              {playerState.loading ? (
              ...
              <div className="absolute left-0 top-0 w-full h-full z-[-1]">
                <div className="absolute backdrop-blur-[200px] bg-black bg-opacity-40 left-0 top-0 w-full h-full"></div>
                <img
